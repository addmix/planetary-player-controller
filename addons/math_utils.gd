...
static func move_to(delta : float, position : float, target : float = 0.0, speed : float = 1.0) -> float:
	var direction : float = sign(target - position)
	var new_position : float = position + direction * speed * delta
...
#returns one vector or the other
static func v3_toggle(condition : bool, _true : Vector3, _false : Vector3) -> Vector3:
	return float(condition) * _true + float(!condition) * _false

#toggles vectors on each axis
static func v3_toggle_axis(condition : Vector3, _true : Vector3, _false : Vector3) -> Vector3:
	return Vector3(float_toggle(bool(condition.x), _true.x, _false.x), float_toggle(bool(condition.y), _true.y, _false.y), float_toggle(bool(condition.z), _true.z, _false.z))
