...
func _physics_process(delta : float) -> void:
	...
	if body_state.total_gravity.length() > 0:
		...
		transform.basis = transform.basis.get_rotation_quaternion().slerp(rotation_basis, delta * 20)
		last_direction = transform.basis.z
	
	#take player input
	var input = Input.get_vector("left", "right", "forward", "backward")
	#map player input to new movement plane
	var trans = _Head.global_transform.basis * Vector3(input.x, 0, input.y)
	#apply movement
	velocity = trans * walkspeed + body_state.total_gravity
	move_and_slide()
